...
from evaluate_model import evaluate_pretrain_model
    ...
    try:
        for run in range(start_run, config['num_runs']):
            ...
            for epoch in range(start_epoch, config['num_epochs']):
                ...
                train_data_loader_tqdm = tqdm(dataloader['pretrain'], ncols=120)
                ...
                val_losses, val_metrics = evaluate_pretrain_model(model=model, evaluate_data_loader=dataloader['pretrain_eval'], loss_func=loss_func, config=config, data_feature= data_feature)

                logger.info(f'Epoch: {epoch + 1}, pretrain duration per epoch: {(time.time() - current_time):.2f} seconds, '
                ...
                for metric_name in val_metrics[0].keys():
                    if metric_name in ['PEARR','accuracy_scores','precision_scores','recall_scores','f1_scores', 'ba',"Loc_acc",
                                       'accuracy_scores_0','precision_scores_0','recall_scores_0','f1_scores_0',
                                       'accuracy_scores_1','precision_scores_1','recall_scores_1','f1_scores_1',
                                       'accuracy_scores_2','precision_scores_2','recall_scores_2','f1_scores_2',
                                       'accuracy_scores_3','precision_scores_3','recall_scores_3','f1_scores_3']:
                ...
                # add val loss as the supervise signal
                val_metric_indicator.append(
                    ('val_loss', np.mean(val_losses), False))
                # count and check the early point epoch, if early stop, save the final epoch in the Try-Finally codes,
                early_stop = early_stopping.step(val_metric_indicator, model)
                if early_stop:
                    break

                early_stopping.save_epoch_checkpoint(model, epoch)
                # in every epoch, save the final epoch parameters in case of the break down in training
            ...
            val_losses, val_metrics = evaluate_pretrain_model(model=model, evaluate_data_loader=dataloader['pretrain_eval'], loss_func=loss_func,
